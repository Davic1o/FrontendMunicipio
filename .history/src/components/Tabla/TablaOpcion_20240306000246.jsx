...
function TablaOpciones({ datos, opcionesBotonAceptar, opcionesBotonCancelar }) {
  ...
  const columnas = Object.keys(datos[0]).filter(columna => columna !== '_id' && columna !== '__v'); // Excluir la columna '_id'

  const handleAceptar = async (_id) => {
    try {
      // Actualizar el usuario en la API y obtén la respuesta
      const updatedUser = await opcionesBotonAceptar.actualizarUsuarioAprobar(_id, 'coordinador');
