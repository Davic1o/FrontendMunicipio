...
function TablaOpciones({ datos, opcionesBotonAceptar, opcionesBotonCancelar }) {
  ...
  const columnas = Object.keys(datos[0]).filter(columna => columna !== '_id' && columna !== '__v'); // Excluir la columna 'ColumnaA'

  const handleAceptar = async (userId) => {
    try {
      // Actualizar el usuario en la API y obtén la respuesta
      const updatedUser = await opcionesBotonAceptar.actualizarUsuarioAprobar(userId, true);
        ...
        {datos.map((fila, index) => (
          <tr className="tr__cuerpo" key={index}>
            ...
            <BotonAceptar
                ...
                onClick={() => handleAceptar(fila.userId)}  {/* Pasa la función de aceptar con el ID del usuario */}
              />
              <BotonCancelar
